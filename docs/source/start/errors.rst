...
X errors all live inside the ``errors`` package:

.. code-block:: python

    from X import errors
...
The ``RPCError`` packs together all the possible errors Telegram could raise, but to make things tidier, X
provides categories of errors, which are named after the common HTTP errors and are subclass-ed from the ``RPCError``:

.. code-block:: python

    from X.errors import BadRequest, Forbidden, ...
...
For a fine-grained control over every single error, X does also expose errors that deal each with a specific
issue. For example:
...
In case X does not know anything about a specific error yet, it raises a generic error from its known category,
