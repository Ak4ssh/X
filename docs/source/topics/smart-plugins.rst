...
X embeds a smart, lightweight yet powerful plugin system that is meant to further simplify the organization
of large projects and to provide a way for creating pluggable (modular) components that can be easily shared across
different X applications with minimal boilerplate code.
...
.. note::
    ...
    Example: *"X"* replies with *"X"* and *"margoryP"*
...
-   ``main.py``
    ...
    .. code-block:: python
        ...
        from X.handlers import MessageHandler
...
manually ``import``, manually :meth:`~X.Client.add_handler` and manually instantiate each
:class:`~X.handlers.MessageHandler` object because you can't use decorators for your functions.
So, what if you could? Smart Plugins solve this issue by taking care of handlers registration automatically.

Using Smart Plugins
-------------------

Setting up your X project to accommodate Smart Plugins is pretty straightforward:
...
The second thing is telling X where to look for your plugins: you can use the Client parameter "plugins";
the *root* value must match the name of your plugins root folder. Your X Client instance will **automatically**
...
:meth:`~X.Client.remove_handler` Client's method with your function's *handler* instance:
...
using :meth:`~X.Client.add_handler` instead. Example:
