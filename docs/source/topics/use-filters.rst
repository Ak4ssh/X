...
Here we'll discuss about :obj:`~X.filters`. Filters enable a fine-grain control over what kind of
...
-   This example will show you how to **only** handle messages containing a :class:`~X.types.Sticker` object and
...
Some filters, like :meth:`~X.filters.command` or :meth:`~X.filters.regex`
...
-   Message is a **text** message or a media **caption** matching the given **regex** pattern.
    ...
    .. code-block:: python
        ...
        @app.on_message(filters.regex("X"))
        async def my_handler(client, message):
            print(message)
...
.. code-block:: python
    ...
    @app.on_message(filters.chat("XChat"))
    async def from_Xchat(client, message):
        print("New message in @XChat")
