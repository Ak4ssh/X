...
Serializing means converting a X object, which exists as Python class instance, to a text string that can be
easily shared and stored anywhere. X provides two formats for serializing its objects: one good looking for
...
.. tip::
    ...
    have done that above just to show you how to explicitly convert a X object to JSON.
...
serialize your object, you can use ``eval()`` to get back the original structure; just make sure to ``import X``,
