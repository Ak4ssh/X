...
Every time you login to Telegram, some personal piece of data are created and held by both parties (the client, X
...
authorization process from scratch each time, X needs to store the generated session data somewhere.

Different Storage Engines
-------------------------

X offers two different types of storage engines: a **File Storage** and a **Memory Storage**.
...
:obj:`~X.Client` constructor, as usual:
...
``my_account.session``. Any subsequent client restart will make X search for a file named that way and the
...
``in_memory`` parameter of the :obj:`~X.Client` constructor:
...
:meth:`~X.Client.export_session_string` anytime before stopping the client...
...
Session strings are useful when you want to run authorized X clients on platforms where their ephemeral
