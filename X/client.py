...
import X
from X import __version__, __license__
from X import enums
from X import raw
from X import utils
from X.crypto import aes
from X.errors import CDNFileHashMismatch
from X.errors import (
    SessionPasswordNeeded,
    VolumeLocNotFound, ChannelPrivate,
    AuthBytesInvalid, BadRequest
)
from X.handlers.handler import Handler
from X.methods import Methods
from X.session import Auth, Session
from X.storage import FileStorage, MemoryStorage
from X.types import User, TermsOfService
from X.utils import ainput
...
class Client(Methods):
    """X Client, the main means for interacting with Telegram.
    ...
    Parameters:
        ...
        in_memory (``bool``, *optional*):
            ...
            :meth:`~X.Client.export_session_string` before stopping the client to get a session string you can
        ...
        workdir (``str``, *optional*):
            Define a custom working directory. The working directory is the location in your filesystem where X
    ...
    async def authorize(self) -> User:
        ...
        print(f"Welcome to X (version {__version__})")
        print(f"X is free software and comes with ABSOLUTELY NO WARRANTY. Licensed\n"
    ...
    def set_parse_mode(self, parse_mode: Optional["enums.ParseMode"]):
        ...
        Parameters:
            parse_mode (:obj:`~X.enums.ParseMode`):
        ...
        if session_empty:
            if not self.api_id or not self.api_hash:
                raise AttributeError("The API key is required for new authorizations. "
                                     "More info: https://docs.X.org/start/auth")
    ...
    async def get_file(
        ...
        try:
            ...
            if not isinstance(e, X.StopTransmission):
                log.error(e, exc_info=True)
