...
import X
from X.methods.utilities.idle import idle
...
class Run:
    def run(
        ...
        :meth:`~X.Client.start`, :meth:`~X.idle` and :meth:`~X.Client.stop` in sequence.
        ...
        operation. This is almost the same as :py:obj:`asyncio.run` except for the fact that X's ``run`` uses the
        current event loop instead of a new one.

        If you want to run multiple clients at once, see :meth:`X.compose`.
