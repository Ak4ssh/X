...
class Object:
    def __init__(self, client: "X.Client" = None):
        self._client = client

    def bind(self, client: "X.Client"):
        """Bind a Client instance to this and to all nested X objects.

        Parameters:
            client (:obj:`~X.types.Client`):
                The Client instance to bind this object with. Useful to re-enable bound methods after serializing and
                deserializing X objects with ``repr`` and ``eval``.
        """
        self._client = client
