...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~X.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~X.Client.sign_in` or :meth:`~X.Client.sign_up`).

        next_type (:obj:`~X.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~X.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~X.Client.resend_code`.
    """
