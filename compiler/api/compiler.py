...
def get_docstring_arg_type(t: str, is_list: bool = False, is_X_type: bool = False):
    if t in CORE_TYPES:
        if t == "long":
            return "``int`` ``64-bit``"
        elif "int" in t:
        ...
        return "Any object from :obj:`~X.raw.types`"
    elif t == "!X":
        return "Any method from :obj:`~X.raw.functions`"
    elif t.lower().startswith("vector"):
        return "List of " + get_docstring_arg_type(t.split("<", 1)[1][:-1], True)
    else:
        return f":obj:`{t} <X.raw.base.{t}>`"
...
def start(format: bool = False):
    ...
    for qualtype in types_to_constructors:
        ...
        items = "\n            ".join([f"- :obj:`{c} <X.raw.types.{c}>`" for c in constructors])
        ...
        for i, arg in enumerate(sorted_args):
            ...
            docstring_args.append(
                "{}{}: {}".format(
                    ...
                    get_docstring_arg_type(arg_type, is_X_type=c.namespace == "X")
                )
            )

        if c.section == "types":
            docstring += f"This object is a constructor of the base type :obj:`~X.raw.base.{c.qualtype}`.\n\n"
        else:
    ...
    with open(DESTINATION_PATH / "all.py", "w", encoding="utf-8") as f:
        ...
        for c in combinators:
            f.write(f'\n    {c.id}: "X.raw.{c.section}.{c.qualname}",')

        f.write('\n    0xbc799737: "X.raw.core.BoolFalse",')
        f.write('\n    0x997275b5: "X.raw.core.BoolTrue",')
        f.write('\n    0x1cb5c415: "X.raw.core.Vector",')
        f.write('\n    0x73f1f8dc: "X.raw.core.MsgContainer",')
        f.write('\n    0xae500895: "X.raw.core.FutureSalts",')
        f.write('\n    0x0949d9dc: "X.raw.core.FutureSalt",')
        f.write('\n    0x3072cfa1: "X.raw.core.GzipPacked",')
        f.write('\n    0x5bb8e511: "X.raw.core.Message",')
